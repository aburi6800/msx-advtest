...
struct saves {
  ...
  void done() { if (p!=0) { while (p!=8) { e*=2; ++p; } addevent(); }  
    FILE *file; 
    if (!(file=fopen(destfilename.c_str(),"wb"))) { cout << "Error writing file!\n"; exit(1); }
    fwrite(buf,1,dp,file); fclose(file); 
    cout << " " << destfilename << ": " << length << " -> " << dp << endl;
  }
} s;
...
void createmetadata() {
  ...
  unsigned r=-1,t=0;
  for (i=length-1;i!=-1;--i) if (d[i]==r) m[i].reeks=++t; else { r=d[i]; m[i].reeks=t=1; }
